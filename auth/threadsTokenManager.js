...
function createThreadsTokenManager({ initialToken, redisUrl, checkIntervalMs = DEFAULT_CHECK_MS, refreshAfterDays = DEFAULT_REFRESH_AFTER_DAYS } = {}) {
  ...
  async function init() {
    if (!redisUrl) {
      console.warn("Threads token manager disabled: REDIS_URL missing");
      return;
    }
    ...
    const refreshedAt = await client.get(REFRESHED_AT_KEY);
    if (!refreshedAt) await client.set(REFRESHED_AT_KEY, String(Date.now()));
  ...
  async function maybeRefresh({ force = false } = {}) {
    ...
    running = true;
    try {
      const lastRaw = await client.get(REFRESHED_AT_KEY);
      const last = Number(lastRaw || 0);
      const ageMs = Date.now() - last;
      const thresholdMs = refreshAfterDays * 24 * 60 * 60 * 1000;
      if (!force && last > 0 && ageMs < thresholdMs) return { refreshed: false, reason: "NOT_DUE" };
      ...
      console.log("Threads token refreshed", JSON.stringify({ ok: true, rotated: nextToken !== currentToken, expiresIn: data?.expires_in || null }));
      return { refreshed: true, rotated: nextToken !== currentToken, expiresIn: data?.expires_in || null };
