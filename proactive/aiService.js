...
function createProactiveAiService({ apiKey, model = "gpt-5.6-luna", state, config, fetchImpl = fetch, timeoutMs = 20000 } = {}) {
  ...
  async function rank(candidates) {
    const items = candidates.slice(0, 25).map(item => ({ id: item.sourcePostId, text: item.text.slice(0, 700) }));
    const messages = [
  ...
  async function draft(candidate, language) {
    const messages = [
      ...
      { role: "user", content: JSON.stringify({ sourcePost: candidate.text.slice(0, 1000), detectedLanguage: language }) },
    ];
