...
function createInstagramCommentPoller({
  ...
  async function readMediaComments(media) {
    if ((Number(media?.comments_count) || 0) <= 0) return { status: "ok", items: [] };
  ...
  async function prime(mediaItems) {
    ...
    for (const media of mediaItems) {
      ...
      const result = await readMediaComments(media);
      if (result.status !== "ok") {
        readFailures += 1;
        continue;
      }
  ...
  async function runOnce() {
    ...
    try {
      ...
      if (!primed) {
        const result = await prime(mediaItems);
      ...
      for (const media of mediaItems) {
        ...
        if (previousCount !== null && currentCount === previousCount) continue;
        if (currentCount <= 0) continue;

        const commentsResult = await readMediaComments(media);
        reads += 1;
      ...
      if (discovered.length || readFailures || failed) {
